import { eq, desc, asc, and, gte, lte, sql, isNull, isNotNull, or, like } from "drizzle-orm";
...
// Função para resetar cache de conexão (útil após erros de quota)
...
export async function getDb() {
  // Forçar recriação do pool para garantir novas credenciais
  if (true) {  // Sempre recriar por enquanto
    try {
      console.log("[Database] Creating new connection pool to TiDB Cloud...");
      
      // Usar novo cluster TiDB "Keyato" (sem throttling)
      const pool = mysql.createPool({
        host: 'gateway01.us-east-1.prod.aws.tidbcloud.com',
        port: 4000,
        user: '41uZEaVYV7KXtRL.root',
        password: 'VX0Nt1hNHzIVXMqg',
        database: 'test',
        ssl: {
